...
namespace dm {
	...
	*/
	...
	/** Perform pre-processing and apply findIris method. Source image is cropped according to r
	* resized to 150x150px and enhanced before it's passed to findIris. Destination matrix contains 
	* annotation regarding iris center and radius.
	*
	* @param src - Source image
	* @param dst - Destination image with annotations
	* @param r - ROI of the eye of the source image 
	* @param upscale - enhanced image of the eye (150x150px)
	* @return vector of int containing coordinates of the center (x[0],y[1]) and radius r[2]
	* of the best iris found, according to upscaled image.
	* 
	*/
	std::vector<int> printIris(cv::Mat src, cv::Mat* dst, cv::Rect r, cv::Mat* upscaled);
